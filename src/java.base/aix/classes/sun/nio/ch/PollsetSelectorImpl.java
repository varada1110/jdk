        ...
        extends SelectorImpl
{
    ...
    private static final int NUM_EPOLLEVENTS = getFDLimit();
	...
	this.pollArrayAddress = Pollset.allocatePollArray(NUM_EPOLLEVENTS);
        ...
        if (Thread.currentThread().isVirtual()) {
            ...
            try {
                ...
                do {
                    ...
                    numEntries = Pollset.pollsetPoll(pfd, pollArrayAddress, NUM_EPOLLEVENTS, to);
    ...
    private int untimedPoll(boolean block) throws IOException {
        int numEntries = Pollset.pollsetPoll(pfd, pollArrayAddress, NUM_EPOLLEVENTS, 0);
        if (block) {
            while (numEntries == 0 && !Thread.currentThread().isInterrupted()) {
                Poller.pollSelector(pfd, 0);
                numEntries = Pollset.pollsetPoll(pfd, pollArrayAddress, NUM_EPOLLEVENTS, 0);
            }
        }
        return numEntries;
    }
    ...
    private int timedPoll(long nanos) throws IOException {
        ...
        while (numEntries == 0 && !Thread.currentThread().isInterrupted()) {
            ...
            Poller.pollSelector(pfd, remainingNanos);
            numEntries = Pollset.pollsetPoll(pfd, pollArrayAddress, NUM_EPOLLEVENTS, 0);
        }
        return numEntries;
    }
                    ...
                    if (newEvents != registeredEvents) {
                        if (newEvents == 0) {
                            ...
                            if (registeredEvents == 0) {
                                ...
                                // modify events
				Pollset.pollsetCtl(pfd, Pollset.PS_MOD, fd, newEvents);
			    }
                        }
